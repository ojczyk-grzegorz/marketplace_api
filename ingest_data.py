...
def main():
    ...
    with open("development/mock_data/users.json") as f:
        users = json.load(f)
        query = insert(DBUser).values(users)
        db.exec(query)

    with open("development/mock_data/items.json") as f:
        items = json.load(f)
        query = insert(DBItem).values(items)
        db.exec(query)

    with open("development/mock_data/discounts.json") as f:
        discounts = json.load(f)
        query = insert(DBDiscount).values(discounts)
        db.exec(query)

    with open("development/mock_data/ground_staff.json") as f:
        ground_staff = json.load(f)
        query = insert(DBGroundStaff).values(ground_staff)
        db.exec(query)

    with open("development/mock_data/delivery_options.json") as f:
